= A simple date-clock application: tutorial cloud-harness

In this small tutorial, we will see different aspects about the developement of applications with cloud harness through the developement from scratch of a small webapp that fetch information from a server on a regular basis.

Cloud harness allows you to quickly setup app that needs to be ran in a kubernete cluster.
...
For the API part, cloud harness relies on OpenAPI 3 to deal with the endpoints/model description.

The different aspects that will be covered here are:

* how to install cloud-harness;
...
* `helm` -- to deal with Kubernete "packages" (helm is basically a package manager for Kubernete)
* `skaffold` -- to build/deploy/run the apps


== Install cloud-harness (if it is not yet done)

The installation is pretty straight forward.
Cloud-harness is coded in Python, consequently, it's always better to create a local virtualenv dedicated to the project to avoid messing with your system dependencies.
...
.Cloning and installing cloud-harness
[source,bash]
----
git clone git@github.com:MetaCell/cloud-harness.git
cd cloud-harness
...
.Activating the virtualenv and installing cloud-harness
[source,bash]
----
...
If everything is good, you're in the right path to use cloud-harness!


== How to setup your local cluster using minikube (if not done yet)

Cloud-harness is designed to help you to generate all the required artifacts for a deployment on a Kebernete cluster.
For a local deployment, you can use minikube, which is basically https://minikube.sigs.k8s.io/docs/start/[a small Kubernete cluster on your machine].
...
Kubernete comes with various addon to deal with various aspects of your cluster.
...
Finally, it's important to activate some environement variables *in each shell you'll use to run commands*.
Those variable will connect the different tools (especially skaffold) to use the docker environement inside your minikube cluster to build your apps images.

.Activating the different environement variables
[source,bash]
----
...
The next snippet show how to generate the `helm` configuration, disabling TLS configuration, enabling the local environement, selecting the `azathoth` namespace to deploy the app, and run it for the `azathoth.local` domain (you can use whatever domain name you want).
...
As you can see, some of those projects are services and not app per-say.
...
.Generatiung the deployment/configuration artifacts for the `samples` app
[source,bash]
----
...
Skaffold will connect to the local docker environnement inside your minikube cluster to build all the images.
...
We use `harness-application` to generate the first files with a specific tempates: `webapp` and `server`.

.Generating the first project's file
[source,bash]
----
# run at the root of the cloud-harness repository
harness-application clockdate -t webapp -t server
----

The name of the application is `clockdate` and we use the `webapp` and `server` template.
There is various existing templates with different purpose: for DB interaction, backend, frontend, ...

We observe now that a new directory had been created in the `applications` folder named `clockdate`.
...
[source,bash]
----
harness-deployment  . -u -dtls -l -d azathoth.local -e local -n azathoth -i clockdate
----
...
.Modifiying the `api/openapi.yaml` file
[source,yaml]
----
paths:
  /currentdate:
...
[NOTE]
The name of the controller in wich the function related to the endpoint will be generated depends on the `tags` value in defined in the `api/openapi.yaml` file.
...
[source,bash]
----
harness-application clockdate -t server -t webapp
----

This will add a new `datetime_controller.py` in the `backend/clockdate/controllers` package.

[IMPORTANT]
You need to notice that all the controllers files (and all the files) are overriden in the `backend` directory.
To prevent files of being overriden, you need to edit the `.openapi-generator-ignore` file, that acts like a `.gitignore` file (in a way), by marking the files/directories that needs to be ignored by the generation.
...
Here a string interpolation is used only to force the result to be considered and formated as a string.
It's not mandatory.
...
<1> The `DatetimeApi` class is instanciated, this is now the instance we will use everytime we need to perform a request toward an API endpoint.
<2> is where is actually perform the call. The `currentdate` method is generated by harness-cloud.

Now that we have our dedicated component, we will integreate it in the current page.
