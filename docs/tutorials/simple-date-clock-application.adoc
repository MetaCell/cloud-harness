...
In this small tutorial, we will see different aspects about the developement of applications with {ch} through the developement from scratch of a small webapp that fetches information from a server on a regular basis.
...
{ch} is designed to help you generate all the required artifacts for a deployment on a Kebernete cluster.
...
We use `harness-application` to generate the first files with a specific tempates: `webapp` and `flask-server`.
