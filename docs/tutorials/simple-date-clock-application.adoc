...
In this small tutorial, we will see different aspects about the developement of applications with CloudHarness through the developement from scratch of a small webapp that fetches information from a server on a regular basis.

CloudHarness allows you to quickly setup app that needs to be ran in a kubernete cluster.
...
For the API part, CloudHarness relies on OpenAPI 3 to deal with the endpoints/model description.
...
The following tools, beside python, are not required to work with CloudHarness.
They are here to deal with the local Kubernete cluster creation, monitoring and the application deployment.
...
CloudHarness is coded in Python, consequently, it's always better to create a local virtualenv dedicated to the project to avoid messing with your system dependencies.
...
First step is to clone the https://github.com/MetaCell/cloud-harness[CloudHarness repository] on your system.
...
CloudHarness is designed to help you generate all the required artifacts for a deployment on a Kebernete cluster.
...
[IMPORTANT]
The setup of a Kubernete cluster is not mandatory to use CloudHarness.
This step is proposed in this tutorial to show you how to deploy an application generated with CloudHarness on a local cluster and to appreciate how much CloudHarness reduces the pain of writing deployment configuration artifacts.
...
[NOTE]
We could place ourself anywere, we would just have to remember the path towards the `cloud-harness` repository.
