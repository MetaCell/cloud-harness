...
def instance_directories(app_path):
    """The instance directories of an application, by instance name."""
    return {app_name_from_path(f"{path.name}"): path
            for path in sorted(instances_path(app_path).glob("*/"))
            if path.is_dir() and not path.name.startswith('.')}


def instance_names(app_name, root_paths):
    """Names of the instances an application declares in any root path."""
    return {name for root_path in root_paths
            for name in instance_directories(Path(root_path) / APPS_PATH / app_name)}
...
def build_instance_values(app_values, parent_name, instance_name, instance_values):
    ...
    """
    app_key = instance_app_key(parent_name, instance_name)
    ...
    # directory's name, so that creating the directory is enough to deploy it. An instance
    ...
    # The volume name is the name of the claim the application mounts: left inherited, the
    # instance would mount the parent's storage. Named after the instance, it gets its own.
    volume = (harness.get(KEY_DEPLOYMENT) or {}).get('volume') or {}
    if volume.get('name') and not instance_sets(instance_values, KEY_HARNESS, KEY_DEPLOYMENT, 'volume', 'name'):
        volume['name'] = f"{app_key}-{volume['name']}"
...
def check_instance_collisions(root_paths, exclude=()):
    ...
    for app_name in applications:
        for instance_name in instance_names(app_name, root_paths):
...
def load_instance_values(instance_path, envs=()):
    ...
    `values-[env].yaml`."""
    values = get_template(instance_path / 'values.yaml')
    for env in envs:
        env_values_path = instance_path / f'values-{env}.yaml'
        if env_values_path.exists():
            with env_values_path.open() as f:
                values = dict_merge(values, yaml.load(f))
    return values
...
def collect_instances(app_name, root_paths, envs=()):
    ...
    `values.yaml` (and optionally `values-[env].yaml`) with the values overriding the
    application's, plus the `resources` and `templates` overriding the application's own. The
    same instance declared in several root paths is merged, a later root overriding an earlier one.
