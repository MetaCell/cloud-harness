...
def test_create_codefresh_configuration():
    ...
    assert step['dockerfile'] == os.path.join(BASE_IMAGES_PATH, "cloudharness-frontend-build", "Dockerfile")
    assert os.path.samefile(step['working_directory'], CLOUDHARNESS_ROOT)

    step = steps["cloudharness-base"]
    assert  os.path.samefile(step['dockerfile'], os.path.join(BUILD_MERGE_DIR, BASE_IMAGES_PATH, "cloudharness-base", "Dockerfile"))
    ...
    assert os.path.samefile(step['working_directory'], os.path.join(STATIC_IMAGES_PATH, "cloudharness-flask"))

    step = steps["my-common"]
    assert step['dockerfile'] == "Dockerfile"
    assert os.path.samefile(step['working_directory'], os.path.join(RESOURCES, STATIC_IMAGES_PATH, "my-common"))
    ...
    step = steps["samples"]
    assert step['dockerfile'] == "Dockerfile"
    assert os.path.samefile(step['working_directory'], os.path.join(APPS_PATH, "samples"))

    step = steps["myapp"]
    assert step['dockerfile'] == "Dockerfile"
    assert os.path.samefile(step['working_directory'], os.path.join(RESOURCES, APPS_PATH, "myapp"))

    step = steps["accounts"]
    assert step['dockerfile'] == "Dockerfile"
    assert os.path.samefile(step['working_directory'], os.path.join(BUILD_MERGE_DIR, APPS_PATH, "accounts"))

    step = steps["workflows-notify-queue"]
    assert step['dockerfile'] == "Dockerfile"
    assert os.path.samefile(step['working_directory'], os.path.join(BUILD_MERGE_DIR, APPS_PATH, "workflows/tasks/notify-queue"))