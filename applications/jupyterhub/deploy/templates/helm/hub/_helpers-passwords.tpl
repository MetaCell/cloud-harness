...
{{- define "jupyterhub.hub.config.ConfigurableHTTPProxy.auth_token" -}}
    {{- if (.Values.apps.jupyterhub.hub.config | dig "ConfigurableHTTPProxy" "auth_token" "") }}
        {{- .Values.apps.jupyterhub.hub.config.ConfigurableHTTPProxy.auth_token }}
    {{- else if .Values.apps.jupyterhub.proxy.secretToken }}
        {{- .Values.apps.jupyterhub.proxy.secretToken }}
    {{- else }}
...
{{- define "jupyterhub.hub.config.JupyterHub.cookie_secret" -}}
    {{- if (.Values.apps.jupyterhub.hub.config | dig "JupyterHub" "cookie_secret" "") }}
        {{- .Values.apps.jupyterhub.hub.config.JupyterHub.cookie_secret }}
    {{- else if .Values.apps.jupyterhub.hub.cookieSecret }}
        {{- .Values.apps.jupyterhub.hub.cookieSecret }}
    {{- else }}
        {{- $k8s_state := lookup "v1" "Secret" .Release.Namespace (include "jupyterhub.hub.fullname" .) | default (dict "data" (dict)) }}
...
{{- define "jupyterhub.hub.config.CryptKeeper.keys" -}}
    {{- if (.Values.apps.jupyterhub.hub.config | dig "CryptKeeper" "keys" "") }}
        {{- .Values.apps.jupyterhub.hub.config.CryptKeeper.keys | join ";" }}
    {{- else }}
        {{- $k8s_state := lookup "v1" "Secret" .Release.Namespace (include "jupyterhub.hub.fullname" .) | default (dict "data" (dict)) }}
...
{{- define "jupyterhub.hub.services.get_api_token" -}}
    {{- $_ := index . 0 }}
    {{- $service_key := index . 1 }}
    {{- $explicitly_set_api_token := or ($_.Values.apps.jupyterhub.hub.services | dig $service_key "api_token" "") ($_.Values.apps.jupyterhub.hub.services | dig $service_key "apiToken" "") }}
